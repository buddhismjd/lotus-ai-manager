...
def test_product_card_contains_required_catalog_fields() -> None:
    ...
    assert "Высота: 14 см" in html
    assert "Материал: бронза" in html
    assert "Под заказ" in html
    assert ">Открыть товар<" in html


def test_product_card_does_not_invent_unknown_status() -> None:
    product = Product(
        id="statue-2",
        title="Статуя Будды",
    ...
    assert "Статус уточняется" not in html
